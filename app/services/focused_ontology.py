...
# Modified Research Paper Aligned Extractor to use focused RAG
class ModifiedResearchPaperAlignedExtractor:
    """Research extractor using only TTL + CEDS RAG"""
    
    def __init__(self):
        self.llm_service = MultiLLMService()
        self.focused_rag = FocusedOntologyRAGService()
        self._initialize_services()
        
    def _initialize_services(self):
        """Initialize focused RAG services"""
        try:
            # Initialize focused RAG (TTL + CEDS only)
            ontology_dir = Path("data/outputs")
            self.focused_rag.initialize_all_sources(ontology_dir)
            
            logger.info("Focused RAG service initialized")
        except Exception as e:
            logger.warning(f"Focused RAG service initialization failed: {e}")

    def _complete_cross_curricular_focused(self, modules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for module in modules:
            if module.get('covers_topics'):
                ...
                for topic in module['covers_topics'][:2]:  # Check first 2 topics
                    try:
                        # Query TTL ontologies for cross-curricular connections
                        ttl_connections = self.focused_rag.find_cross_curricular_connections(topic)
                        all_connections.extend(ttl_connections)
                        
                        # Query CEDS for curriculum alignment
                        ceds_connections = self.focused_rag.query_ceds_for_curriculum_alignment(
                ...
                if all_connections:
                    module['cross_curricular_connections'] = all_connections[:3]  # Top 3
                    module['rag_source'] = 'ttl_and_ceds_only'
        
        return modules

    def _generate_persona_learning_paths_with_ttl_rag(self, 
                                                     text: str, 
                                                     personas: List[PersonaType],
                                                     provider: LLMProvider) -> List[Dict[str, Any]]:
        """Generate learning paths using TTL ontology patterns"""
        
        learning_paths = []
        
        for persona in personas:
            # Get existing patterns from TTL ontologies only
            existing_patterns = self.focused_rag.get_learning_path_patterns(persona.value)
            
            # Format RAG context from TTL files
            rag_context = "EXISTING LEARNING PATH PATTERNS FROM TTL ONTOLOGIES:\n"
            for pattern in existing_patterns[:3]:
                rag_context += f"From {pattern['source_curriculum']} ontology:\n"
                for step in pattern['steps']:
                    rag_context += f"  - {step['description']}\n"
                rag_context += "\n"
            
            if not existing_patterns:
                rag_context += "No existing patterns found in TTL ontologies.\n"
            
            # Same LLM prompt as before but with TTL-focused context
            prompt = f"""Create learning path for {persona.value} persona using curriculum content:

CURRENT CURRICULUM: {text}
...
{{
    ...
    "learning_steps": [
        {{
            "step_number": 1,
            "is_first": true,
            "title": "Step title from curriculum",
            "description": "What the learner does",
            "refers_to_module": "Actual module name from curriculum",
            "estimated_duration": "30 minutes",
            "has_next_step": 2
        }}
    ],
    "ttl_rag_patterns_used": {len(existing_patterns)}
}}"""
            ...
            try:
                ...
                logger.error(f"TTL RAG learning path generation failed for {persona}: {e}")
